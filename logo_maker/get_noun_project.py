...
def get_icon_ids(keyword : str, limit : int) -> list[str]: 
    parsed_data = requests.get("https://api.thenounproject.com/v2/icon?query=" + keyword + "&limit=" + limit + "&thumbnail_size=200&include_svg=0&limit_to_public_domain=1", auth=auth)
    print(parsed_data)
